...
class ClosedFormSampledDistanceLoss(nn.Module):
    ...
    def _compute_prob_matching_loss(self, logits, matched, smoothness=0):
        ...
        matched.to(logits.device)
        loss = self.bceloss(logits, matched.to(logits.device))
    ...
    def _compute_closed_form_loss(self, input1, input2, matched, smoothness=0):
        ...
        loss_dict = self._compute_prob_matching_loss(logits, matched.to(logits.device), smoothness=smoothness)
    ...
    def forward(self, img, cap, matched=None):
        img_emb={}
        cap_emb={}
        img_emb['mean']=img[0]
        img_emb['std']=torch.log(img[1].exp().pow(0.5))
        cap_emb['mean']=cap[0]
        cap_emb['std']=torch.log(cap[1].exp().pow(0.5))
